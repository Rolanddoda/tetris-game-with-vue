...
export default {
	...
	methods: {
		...
		game_flow() {
			...
			for (let ar_row = pos, t_row = rows_count; ar_row >= 0 && t_row >= 0; ar_row--, t_row--) { //ar_row = arena row,
				arena[ar_row] = this.merge_rows(arena[ar_row], this.new_tetris_item.value[t_row])
			}
		...
		clear_arena(arena) { //clear arena from active tetris row (active tetris has number 2)
		...
		remove_active_tetris() { //tetris reached the end so make the rows containing number 2 to become 1
		...
		rotate_matrix(original_matrix) { //rotate the matrix 90 deg
		...
		clear_matrix(matrix) { //remove first and/or last row if is full of 0
		...
		tetris_rotated(matrix) {
			...
			let rotated_tetris = []
			if (starts + expands > 9) { starts -= starts + expands - 9 }
		...
		change_tetris_position(pos) { //pos => position can be -1 or 1
		...
		check_for_score(arena) {
			...
			indexes_to_clear.forEach(index => c_arena.push([0, 0, 0, 0, 0, 0, 0, 0, 0]))
